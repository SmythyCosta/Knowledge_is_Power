...
echo '<h4>Páginas que você visualizou</h4>';

if (mysqli_num_rows($r) > 0) {
	
	while ($row = mysqli_fetch_array($r, MYSQLI_NUM)) {
		// Display each record:
		echo "<div><h4><a href=\"page.php?id=$row[0]\">$row[1]</a></h4><p>$row[2]<br />última visita: $row[3]</p></div>\n";
	}

} else { // No pages available.
	echo '<p>Você ainda não viu nenhuma página.</p>';
}
...
echo '<h4>PDFs que você viu</h4>';

if (mysqli_num_rows($r) > 0) {
	
	while ($row = mysqli_fetch_array($r, MYSQLI_NUM)) {
		// Display each record:
		echo "<div><h4><a href=\"view_pdf.php?id=$row[0]\">$row[1]</a></h4><p>$row[2]<br />última visita: $row[3]</p></div>\n";
	}

} else { // No pages available.
	echo '<p>Você ainda não visualizou nenhum PDF.</p>';
}
